    ...
    <div className="min-h-screen bg-[#F5F5FA] flex flex-col font-sans text-gray-800">
      ...
      <main className="flex-grow flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl w-full">
          <div className="text-center mb-10">
            <h1 className="text-3xl font-black text-gray-900 mb-4">Ubah Gambar ke PDF</h1>
            <p className="text-gray-600">Konversi JPG dan PNG Anda menjadi dokumen PDF berkualitas tinggi secara instan.</p>
          </div>
          ...
          <div className="relative group mb-10">
            ...
            >
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                ...
                <p className="mb-2 text-lg font-bold text-gray-700">Klik untuk unggah gambar atau seret di sini</p>
                <p className="text-sm text-gray-500 font-medium">Hanya JPG, JPEG, dan PNG yang diperbolehkan</p>
              </div>
          ...
          {previews.length > 0 && (
            <div className="mb-10">
              <h2 className="text-lg font-bold text-gray-900 mb-6">Pratinjau Gambar ({images.length})</h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {previews.map((url, index) => (
                  <div key={index} className="relative group aspect-square rounded-xl overflow-hidden border border-gray-200 bg-white shadow-sm">
                    <img src={url} alt={`Preview ${index}`} className="w-full h-full object-cover" />
                    <button 
                      onClick={() => removeImage(index)}
                      className="absolute top-1 right-1 bg-white/90 hover:bg-red-600 hover:text-white text-gray-600 p-1.5 rounded-full shadow-md transition-all opacity-0 group-hover:opacity-100"
                    >
                      <X className="w-4 h-4" />
                    </button>
                    <div className="absolute bottom-0 left-0 right-0 bg-black/40 text-white text-[10px] py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <footer className="w-full bg-white border-t border-gray-200 py-8 mt-auto">
        <div className="max-w-7xl mx-auto px-4 text-center text-sm font-medium text-gray-500">
          Privasi Anda penting. Semua konversi gambar diproses secara lokal.
        </div>
      </footer>
    </div>
  );
