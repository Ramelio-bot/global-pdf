    ...
    try {
      ...
      for (let i = 0; i < slideFiles.length; i++) {
        ...
        if (slideXml) {
          ...
          // Get PowerPoint text elements <a:t>
          ...
          for (let j = 0; j < textNodes.length; j++) {
            ...
            if (text && text.trim()) {
              ...
              if (y > 190) {
                // If text is too long for one PDF slide page
                y = 30;
                doc.addPage();
              }
            }
      ...
      alert("An error occurred while converting the PowerPoint file. Please ensure the file is not corrupted.");
    } finally {
      ...
      <main className="flex-grow flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl w-full">
          <div className="text-center mb-10">
            <h1 className="text-3xl font-black text-gray-900 mb-4 tracking-tight">Convert PPT to PDF</h1>
            <p className="text-gray-600">Extract text from PowerPoint slides (.pptx) and save them as a clean PDF document.</p>
          </div>
            ...
            >
              {file ? (
                <div className="flex flex-col items-center p-4 text-center animate-in zoom-in-95">
                  <div className="bg-orange-100 p-4 rounded-full mb-4 shadow-inner">
                ...
                <div className="flex flex-col items-center justify-center pt-5 pb-6 group-hover:scale-105 transition-all duration-300">
                  <div className="bg-gray-100 p-4 rounded-full mb-4 group-hover:bg-orange-50 transition-colors shadow-inner">
                    <FileUp className="w-10 h-10 text-gray-400 group-hover:text-orange-500 transition-colors" />
                  </div>
                  <p className="mb-2 text-lg font-bold text-gray-700 tracking-tight">Click or drag PowerPoint file (.pptx) here</p>
                  <p className="text-sm text-gray-500 font-medium italic">Local & private processing</p>
                </div>
              )}
          ...
          {isDone && (
            <div className="mb-8 bg-green-50 border border-green-100 p-6 rounded-2xl flex items-center gap-4 animate-in fade-in slide-in-from-bottom-4">
              ...
              <div>
                ...
                <p className="text-sm text-green-700 font-bold">Your PDF document has been downloaded automatically.</p>
              </div>
            </div>
          )}
